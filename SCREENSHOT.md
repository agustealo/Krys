...
## Documentation runtime captures

The repository also keeps reproducible runtime screenshots under `docs/images/`. Those images are generated by `.github/workflows/capture-doc-screenshots.yml` from a temporary real WordPress 7.1.1 + WooCommerce installation and are used in the GitHub README and relevant documentation.

See [`docs/SHOWCASE.md`](docs/SHOWCASE.md) for the current homepage, shop, cart, checkout, and mobile captures plus their provenance. These documentation captures are evidence of the running theme, but they do not replace the manually curated WordPress.org `screenshot.png` release asset.
